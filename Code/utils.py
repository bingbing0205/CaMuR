...
class scRNADataset(Dataset):
    ...
    # 此函数生成的不是一种对称的图仅仅是关联对都为转录因子时，生成的图才是对称的图
...
# def construct_PPI_network(num_gene, interactions):
#     ppi = sp.dok_matrix((num_gene, num_gene), dtype=np.float32)
...
def Evaluation(y_true, y_pred,flag=False):
    # print("Pred Preview:", y_pred[:5]) # 打印前5个预测值
    # print("Pred Std:", torch.std(y_pred).item()) # 打印标准差
    if flag:
